...
- [ ] 7.1 Commit the work on `feat/folder-preferences-security-bookmarks` as conventional commits grouped by concern (configuration break, preferences document, bookmark lifecycle, registry, settings surface, docs/gate). Verify: `git log` shows the grouped history and the worktree diff is fully committed.
