...
The system SHALL durably append and synchronize each accepted transition before making it observable to its caller. It SHALL accept only `discovered -> archived -> hashed -> queued -> uploading -> uploaded -> confirmed` and SHALL retain an idempotency key derived deterministically from the SHA-256 digest. For a queued or uploading entry, it SHALL also retain only the non-secret resumption token, received chunk indices or equivalent acknowledged progress, retry attempt count, retry eligibility, and queue-control state necessary to resume safely. When an entry is bound to a Platform import operation, it SHALL additionally retain only the operation id, last valid privacy-safe operation/completeness projection, its observation time, and delivered terminal-notification identity.
...
#### Scenario: Last backend observation survives restart
- **WHEN** a valid processing or terminal import observation is persisted and the process restarts
- **THEN** the entry retains the observed status and timestamp without persisting backend diagnostics or archive content
