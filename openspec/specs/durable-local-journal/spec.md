...
The system SHALL durably append and synchronize each accepted transition before
making it observable to its caller. It SHALL accept only `discovered ->
archived -> hashed -> queued -> uploading -> uploaded -> confirmed` and SHALL
retain an idempotency key derived deterministically from the SHA-256 digest.
